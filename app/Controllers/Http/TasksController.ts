...
export default class TasksController {
  public async index({ view, auth, params }: HttpContextContract) {
    ...
    const userToShare = await User.query().where('id', '<>', auth.user!!.id )
    return view.render('tasks/index', { tasks, list, userToShare })
  }
  ...
  private validate(data, session):Boolean {
    ...
    if(data.description === data.title){
      this.registerError(errors, 'description', 'descrição não pode ser igual ao titulo')
    }
